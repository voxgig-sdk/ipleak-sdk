...
boru-driven command-line client **and** interactive REPL for the Ipleak
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ipleak-cli)
make build
...
./ipleak-cli load 1 ip_information            # {id:1} shorthand
./ipleak-cli load '{id:1}' ip_information       # explicit match map

# 5. Override the API base URL for a single call
IPLEAK_BASE=https://api.example.com ./ipleak-cli load 1 ip_information

# 6. No arguments -> interactive REPL
./ipleak-cli
ipleak> load 1 ip_information
ipleak> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ipleak-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ipleak-cli load 1 ip_information
   ```
...
```sh
./ipleak-cli load 1 ip_information          # scalar shorthand for {id:1}
./ipleak-cli load '{id:1}' ip_information     # explicit match map
```
...
./ipleak-cli load 1 ip_information
```
...
Run with no arguments to open a REPL (prompt `ipleak>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ipleak-cli`. |
...
(`./ipleak-cli <expr>`) and interactively (the REPL), and expressions compose the
