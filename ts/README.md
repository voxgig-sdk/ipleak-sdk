# Ipleak TypeScript SDK



The TypeScript SDK for the Ipleak API — a type-safe, entity-oriented client with full async/await support.
...
- Releases: [https://github.com/voxgig-sdk/ipleak-sdk/releases](https://github.com/voxgig-sdk/ipleak-sdk/releases)
...
IpInformation is nested under ip, so provide the `ip`.
...
| `tester(testopts?, sdkopts?)` | `IpleakSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `IpleakSDK.test(testopts?, sdkopts?)` | `IpleakSDK` | Create a test-mode client. |
...
| `client` | `client(): IpleakSDK` | Return the parent SDK client. |
...
```ts
const ip_information = await client.IpInformation().load({ ip: 'ip' })
```
