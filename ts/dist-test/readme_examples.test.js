...
function isRunnable(code) {
    return (/new\s+IpleakSDK\b/.test(code) ||
        /\bIpleakSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IpleakSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IpleakSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
