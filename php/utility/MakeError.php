...
class IpleakMakeError
{
    public static function call(?IpleakContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpleakContext([], null);
        }
        $op = $ctx->op ?? new IpleakOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpleakResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpleakError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpleakError('', $msg, $ctx);
