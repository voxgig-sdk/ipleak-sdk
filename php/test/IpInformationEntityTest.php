...
class IpInformationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set IPLEAK_TEST_IP_INFORMATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function ip_information_basic_setup($extra)
{
    ...
    $client = IpleakSDK::test($options, $extra);

    // Generate idmap.
    $idmap = [];
    foreach (["ip_information01", "ip_information02", "ip_information03", "json01", "json02", "json03"] as $k) {
    ...
    $entid_env_raw = getenv("IPLEAK_TEST_IP_INFORMATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["IPLEAK_TEST_IP_INFORMATION_ENTID"]);
    ...
    if ($env["IPLEAK_TEST_LIVE"] === "TRUE") {
        ...
        $client = new IpleakSDK(Helpers::to_map($merged_opts));
    }
