...
class FtCtrl extends IpleakControl
{
...
class FtClient
{
    ...
    private IpleakUtility $utility;

    public function __construct(IpleakUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpleakUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpleakContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpleakContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpleakConfig::make_feature($name);
    ...
    private static function buildUrl(IpleakSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpleakError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpleakResponse($fetched);
        }
    ...
    private function fail(IpleakContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpleakContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
