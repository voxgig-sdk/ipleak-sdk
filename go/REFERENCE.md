# Ipleak Golang SDK Reference

Complete API reference for the Ipleak Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *IpleakSDK`
...
#### `IpInformation(data map[string]any) IpleakEntity`
...
```go
result, err := client.IpInformation(nil).Load(map[string]any{"ip": "ip"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
