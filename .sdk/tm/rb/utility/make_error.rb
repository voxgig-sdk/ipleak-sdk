...
module IpleakUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IpleakContext.new({}, nil)
    end
    op = ctx.op || IpleakOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IpleakResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IpleakError) ? err.msg : err.to_s
    ...
    sdk_err = IpleakError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(IpleakError)
