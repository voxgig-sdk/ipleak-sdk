...
//    only - a typed IpleakContext there would make every ctx
...
final class IpleakOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpleakContext($clean, null);
    ...
    public static function err_from_map($m): ?IpleakError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpleakError(is_string($code) ? $code : '', $msg);
    }
