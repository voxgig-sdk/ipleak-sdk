// Command Ipleak-mcp is an MCP server exposing the Ipleak
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Ipleak_list  — list records of an entity
//	Ipleak_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: IPLEAK_APIKEY carries the API key and
	// IPLEAK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IPLEAK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IPLEAK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpleakSDK(opts)
