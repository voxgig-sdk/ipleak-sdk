...
[MCP](https://modelcontextprotocol.io) server exposing the Ipleak SDK as
two agent tools — `ipleak_list` and `ipleak_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ipleak-mcp)
make build

# 2. Provide credentials via the environment
export IPLEAK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ipleak \
  -- /absolute/path/to/ipleak-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ipleak-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ipleak_list: first page of records
{ "entity": "ip_information" }
{ "entity": "ip_information", "query": { } }

// ipleak_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ipleak-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ipleak \
     -- "$PWD"/dist/*/ipleak-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ipleak_list` and `ipleak_load` tools now appear
   in new sessions. Ask the agent to *"list ip_information using ipleak"*
   and it calls `ipleak_list` with `{"entity":"ip_information"}`.
...
```sh
export IPLEAK_APIKEY=sk_live_xxx            # API key
export IPLEAK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `ipleak_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ipleak_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IPLEAK_APIKEY` | API key sent with every request. |
| `IPLEAK_BASE` | Optional override of the API base URL. |
...
```sh
./ipleak-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ipleak_load","arguments":{"entity":"ip_information","query":{"id":1}}}}'
```
