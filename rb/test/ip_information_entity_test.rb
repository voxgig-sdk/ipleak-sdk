...
class IpInformationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPLEAK_TEST_IP_INFORMATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def ip_information_basic_setup(extra)
  ...
  entid_env_raw = ENV["IPLEAK_TEST_IP_INFORMATION_ENTID"]
  ...
  if env["IPLEAK_TEST_LIVE"] == "TRUE"
    ...
    client = IpleakSDK.new(Helpers.to_map(merged_opts))
  end
