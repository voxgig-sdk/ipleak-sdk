...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = IpleakOmni.make_runner(TEST_JSON_FILE, IpleakSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : IpleakOmni.errify(err)
    end
